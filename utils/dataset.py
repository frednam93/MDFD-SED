...
class StronglyLabeledDataset(Dataset):
    def __init__(self, tsv_read, dataset_dir, return_name, encoder, mapping=None):
        ...
        if mapping is not None:
            mappings = {}
            mapping_read = pd.read_csv(mapping, sep="\t")
            for _, row in mapping_read.iterrows():
                if row["original_file"] not in mappings.keys():
                    mappings[row["original_file"]] = os.path.join(dataset_dir, row["public_file"])
    ...
    def __getitem__(self, idx):
        ...
        if self.mappings is None:
            ...
            wav, pad_mask = wav_loader(self.mappings[filename], self.pad_to, self.encoder)
...
def add_two_wav(wav1, wav2, r):
    return normalize_wav(normalize_wav(wav1) + r * normalize_wav(wav2))

...
def data_augs(train_cfg, wavs, mels, labels, weak_labels=None, mask_strong=None, mask_weak=None):
...
def frame_shift(features, label=None, net_pooling=None):
    if label is not None:
        ...
        for idx in range(batch_size):
            ...
            # shift = int(random.randint(0, n_frames))
            # shift = int(torch.normal(torch.Tensor([0]), torch.Tensor([90])).item()) # using torch causes
        ...
        for idx in range(batch_size):
            shift = int(random.gauss(0, 90))
            # shift = int(random.randint(0, n_frames))
            # shift = int(torch.normal(torch.Tensor([0]), torch.Tensor([90])).item())
            shifted_feature.append(torch.roll(features[idx], shift, dims=-1))
...
def feature_transformation(features, n_transform, choice, filter_db_range, filter_bands, min_bw, filter_type,
    ...
    if n_transform == 2:
        ...
        return feature_list
    elif n_transform == 1.5:
        # one augmented feature, another not augmented feature
        ...
        return [features, features_original]
    elif n_transform == 1:
